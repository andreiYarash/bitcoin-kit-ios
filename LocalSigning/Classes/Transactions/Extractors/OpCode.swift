...
public class OpCode_Local_Usage {
    public static let p2pkhStart = Data([OpCode_Local_Usage.dup, OpCode_Local_Usage.hash160])
    public static let p2pkhFinish = Data([OpCode_Local_Usage.equalVerify, OpCode_Local_Usage.checkSig])

    public static let p2pkFinish = Data([OpCode_Local_Usage.checkSig])

    public static let p2shStart = Data([OpCode_Local_Usage.hash160])
    public static let p2shFinish = Data([OpCode_Local_Usage.equal])
    ...
    public static func push(_ data: Data) -> Data {
        ...
        case 0x4c...0xff: bytes = Data([OpCode_Local_Usage.pushData1]) + UInt8(length).littleEndian
        case 0x0100...0xffff: bytes = Data([OpCode_Local_Usage.pushData2]) + UInt16(length).littleEndian
        case 0x10000...0xffffffff: bytes = Data([OpCode_Local_Usage.pushData4]) + UInt32(length).littleEndian
        default: return data
        }
    ...
    public static func scriptWPKH(_ data: Data, versionByte: Int = 0) -> Data {
        return OpCode_Local_Usage.push(versionByte) + OpCode_Local_Usage.push(data)
    }
